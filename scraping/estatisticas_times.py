...
class Estatisticas_times:
    ...
    def atualizar_estats():
        ...
        for _, row in df_filtrado.iterrows():
            cursor.execute("""
                IF EXISTS (SELECT 1 FROM EstatisticasTimes WHERE Equipe = ?)
                ...
                ELSE
                    INSERT INTO EstatisticasTimes (
                        ...
                        GC90, ChutesGC, SemVazar, PercentualSV, PT, GPC, PSV, GPp, PercentualDefesas, 
                        GCFalta, GCEscanteio, GolsContra, TotalChutes, ChutesG, ChutesPercentual, 
                        TotalChutes90, ChutesGol90, PassesCompletos, PassesTentados, PercentualPasses, 
                        FaltasCometidas, FaltasProvocadas, Cruzamentos, PenaltisConvertidos, PenaltisConcedidos, 
                        RecuperacaoBola, GanhosAereo, PerdasAereo, GanhosPercentual
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                # UPDATE (36 parâmetros)
                row['Squad'],  # 1 - IF EXISTS
                float(row['Poss']),
                int(row['CrdY']),
                int(row['CrdR']),
                ...
                row['Squad'],  # WHERE Equipe (36º parâmetro)

                # INSERT (35 parâmetros)
                row['Squad'],
                float(row['Poss']),
                int(row['CrdY']),
                int(row['CrdR']),
